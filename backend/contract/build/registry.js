...
var getSymbols = !nativeGetSymbols$1
	...
	: (object) => {
			...
			return arrayFilter(nativeGetSymbols$1(object), (symbol) =>
...
function log(...params) {
	env.log(
		params.reduce((accumulated, parameter, index) => {
			// Stringify undefined
...
class Vector extends SubType {
	...
	swapRemove(index, options) {
		...
		assert(
			storageWriteRaw(bytes(key), serializeValueWithOptions(last, options)),
...
const Registry =
	...
	_dec(
		(_class =
			((_class2 = class Registry {
				records = new Vector("r");
				add_record({ did1, hash, did2 }) {
			...
			_applyDecoratedDescriptor(
				...
				"add_record",
				[_dec2],
				Object.getOwnPropertyDescriptor(_class2.prototype, "add_record"),
				_class2.prototype,
			),
			_applyDecoratedDescriptor(
				_class2.prototype,
				"get_records",
				[_dec3],
				Object.getOwnPropertyDescriptor(_class2.prototype, "get_records"),
				_class2.prototype,
			),
			_class2)),
	) || _class);
function get_records() {
	...
	if (_result !== undefined)
		if (
			...
			_result.onReturn();
		else env.value_return(Registry._serialize(_result, true));
}
function add_record() {
	const _state = Registry._getState();
	if (!_state && Registry._requireInit()) {
